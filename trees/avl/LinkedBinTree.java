...
public class LinkedBinTree<E> extends AbstractBinaryTree<E> {
	 ...
	 * one line per node.
	 */
	private void printTree_r(Node<E> node, int depth) { 
		if(node==null) { 
			return;
		} else {
	...
	/** Clone a binary tree - public version. */
	public LinkedBinTree<E> clone() { 
		LinkedBinTree<E> btree = new LinkedBinTree<E>();
		btree.root = clone_r(this.root);
		return btree;
	}


	/** Clone a node of a binary tree - recursively. */
	protected Node<E> clone_r(Node<E> node) {
		if(node==null) { 
			return null;
		} else {
			return new Node<E>(
					node.getElement(),
					null,
					clone_r(node.getLeft()),
					clone_r(node.getRight()));
		}
	}
